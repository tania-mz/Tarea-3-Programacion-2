...
double GetCosh(double x, int limit, int k) {
    if (k <= limit) { // Mientras el limite inferior no sea mayor al limite superior se continúa con la recursión
        return (pow(x, 2 * k) / CalculateFactorial(2 * k, 1) ) + GetCosh(x, limit, k + 1); // Llamado recursivo
...
int main() {
    ...
    printf("Ingrese el numero de terminos con los que desea calcular: ");
    scanf("%d", &limit);

    // Llamamos a la función GetSenh e imprimimos el valor que devuelve
    printf("%lf", GetCosh(x, limit, 0));
